...
import { getChain } from "@/utils/helper";
...
export const SelectorCard = ({
  ...
  return (
    <>
        ...
        <div
          ...
          <div className="flex w-full justify-between items-end px-4 text-white">
            ...
            <div className="flex flex-col items-end text-white-600">
              ...
              <div className="flex items-center space-x-2 cursor-pointer" onMouseEnter={() => setNetworkActive(true)}>
                <Ethereum/>
                <h6>{getChain(isSendChain ? connectedChainId : chainId).name}</h6>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col border border-white border-opacity-20 py-3 px-4 space-y-4 w-full">
            <div className="flex items-center justify-between w-full text-xs">
              <span className="text-[#9CA3AF]">Balance: {Number(result?.data?.formatted).toFixed(2)} {getChain(isSendChain ? connectedChainId : chainId).nativeCurrency.symbol}</span>
              {
                isSendChain &&
                  <button className="text-[#60A5FA]" onClick={onMaxClick}>Select Max</button>
              }
            </div>
            <div className="flex items-center justify-between w-full">
              <div className="flex items-center space-x-2" onMouseEnter={() => setActive(true)}>
                <Ethereum />
              ...
              {
                isSendChain ?
                  <Input className="text-white border-none bg-transparent text-right" type="number" value={amount} onChange={(e) => setAmount ? setAmount(Number(e.target.value)) : null} />
                  :
      ...
      <CurrencySelect active={active} setActive={setActive} currency={currency} setCurrency={(value) => {
        setCurrency(value)
        setActive(false)
      }}/>
      <NetworkSelect active={networkActive} setActive={setNetworkActive} chainId={chainId} setChainId={(chainId) => {
        setChainId(chainId)
